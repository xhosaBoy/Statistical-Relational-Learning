...
\noindent RESCAL, by Nickel et al.  \unskip~\citep{nickel2011three}, decomposes a relational tensor $ T_k $ into an entity matrix $ E  \in \mathbb{R}^{n x r} $, a relational tensor $ R_k \in \mathbb{R}^{r x r x m}$ and the transpose of the entity matrix $ E^T \in \mathbb{R}^{r x n} $. The bilinear tensor product is the inner product between $ E $ and a slice $ k $ of $ R_k $, where the inner product of the result is taken with a transposed matrix $ E^T $, see equation 1.1.
\begin{equation}
	T_k \approx ER_kE^T
\end{equation}

\noindent Entity matrix $ E $ is composed of entity vectors $ e \in \mathbb{R}^r $ and a relation matrix $ k $ of the relational tensor $ R_k $ is represented as a full rank matrix $ W_k \in \mathbb{R}^{rxr} $ \unskip~\citep{nickel2012factorizing}.\ An inner product between an entity $ e_i $ and relational matrix $ W_k $, followed by an inner product with entity vector $ e_j $ generates a relational score $ n_{i,j,k} $, see Figure 1.4. When adopting the RDF formalism, $ e_i $ is the subject, $ W_k $ is the predicate, and $ e_j $ is the object.\ This set comprises an RDF triple used to encode a KG fact.\ Entities $ e_i $, and $ e_j $ and relation $ W_k $ have unknown latent representations (parameters), and the task is to learn the most optimal parameters that can be used for relation prediction. The training algorithm minimises the squared error loss, and the model achieves competitive performance on various datasets. \par

\begin{figure}[H]
   	\centering
    	\includegraphics[width=0.9\textwidth, height=0.4\textwidth]{relational_tensor_entity_relational_factorisation}
	\captionsetup{justification=centering}
	\caption{Entity-relational tensor decomposition into entity matrix $ E  \in \mathbb{R}^{n x r} $,  relational tensor $ R_k \in \mathbb{R}^{r x r x m} $ and the transposed entity matrix $ E^T \in \mathbb{R}^{r x n} $. The inner product of these matrices and tensor compute the entity-relational tensor.}
\end{figure}
